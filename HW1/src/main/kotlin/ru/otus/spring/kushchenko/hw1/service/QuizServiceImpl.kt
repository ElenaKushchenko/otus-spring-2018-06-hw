...
class QuizServiceImpl(private val repository: QuestionRepository) : QuizService {
    ...
    override fun checkAnswer(questionId: Int, answerNum: Int): Boolean {
        log.debug("Check answer: question = $questionId, answer = $answerNum")
        val question = repository.getById(questionId)
        return if (question != null) {
            question.correctAnswerNum == answerNum
        } else {
